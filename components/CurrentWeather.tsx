...
export const CurrentWeather: React.FC<CurrentWeatherProps> = ({ data, unit, theme }) => {
  ...
  const subTextColor = isDark ? 'text-white/80' : 'text-slate-600';
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center py-12 md:py-20 relative z-10">
        ...
        <h2 className={`text-3xl md:text-5xl font-medium tracking-tight drop-shadow-lg mb-4 ${textColor}`}>
          {location.city}
        </h2>
        
        {/* Massive Temperature Display */}
        <div className="relative flex items-center justify-center">
            <span className={`text-[8rem] md:text-[11rem] leading-none font-thin tracking-tighter text-transparent bg-clip-text bg-gradient-to-b ${tempGradient} drop-shadow-2xl select-none`}>
                {temp}°
            </span>
        </div>
        
        {/* Condition Text */}
        <div className={`text-xl md:text-3xl font-medium capitalize drop-shadow-md mt-4 tracking-wide ${textColor}`}>
            {current.description}
        </div>
